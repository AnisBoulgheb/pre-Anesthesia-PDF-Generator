...
app.post("/api/generate-pdf", (req, res) => {
  ...
  const tableRow = (items, ty, rh) => {
    ...
    items.forEach(({ lw, vw }) => {
      if (lw > 0) {
        doc.lineWidth(0.2).strokeColor("black").rect(pt(cx), pt(ty), pt(lw), pt(rh)).stroke();
      }
      if (vw > 0) {
        doc.lineWidth(0.2).strokeColor("black").rect(pt(cx + lw), pt(ty), pt(vw), pt(rh)).stroke();
      }
      cx += lw + vw;
    });
    ...
    items.forEach(({ label, val, lw, vw, unit }) => {
      ...
      let alignLbl = "C";
      if (["Poids:", "Taille:", "Conjonctives:", "Etat Général:", "TABAC:", "NYHA:", "AUTRE"].includes(label)) {
        alignLbl = "L";
      }
      ...
      if (vw > 0) {
        if (label === "Plaquettes:" && val && !String(val).includes("000/mm3")) {
        ...
        if (unit) {
          F("normal", 5.5);
          doc.fillColor("black").text(unit, pt(cx + lw), pt(ty + rh - 2.5), { width: pt(vw - 1), align: "right" });
        }
      }
      cx += lw + vw;
    });
    ...
    // Outer border
    doc.lineWidth(0.4).strokeColor("#000000").rect(pt(MG), pt(ty), pt(CW), pt(rh)).stroke();
  };
  ...
  atcdLabels.forEach((lbl, i) => {
    // Cell borders
    doc.lineWidth(0.2).strokeColor("black").rect(pt(MG), pt(y), pt(aLW), pt(aH)).stroke();
    doc.lineWidth(0.2).strokeColor("black").rect(pt(MG + aLW), pt(y), pt(aVW), pt(aH)).stroke();
    ...
    // Outer border
    doc.lineWidth(0.4).strokeColor("#000000").rect(pt(MG), pt(y), pt(CW), pt(aH)).stroke();
  ...
  tableRow(
    [
      ...
      { label: "Plaquettes:", val: d.plaquettes || "", lw: 22, vw: 28, unit: "000/mm3" },
  ...
  put("Conclusion & consignes:", MG, y);

  if (d.conclusion && d.conclusion.trim()) {
    y += 4;
    F("normal", 8);
    doc.fillColor("black");
    const opts = { width: pt(CW), align: "left" };
    doc.text(d.conclusion.trim(), pt(MG), pt(y), opts);
    y += doc.heightOfString(d.conclusion.trim(), opts) / 2.834645 + 2;
  } else {
    hline(MG + 42, PAGE_W - MG, y + 3.2);
    y += 5;
    hline(MG, PAGE_W - MG, y + 3.2);
    y += 5;
  }
